...
public class FinishFactory implements EntityFactory {
        ...
        public Entity createEntity(Level level, JSONObject config) {
                try {
                        ...
                        Optional<Double> height = Optional.ofNullable(((Number) config.getOrDefault("height", null)))
                                        .map(Number::doubleValue);

                        String imageName = (String) config.getOrDefault("image", "tree.png");

                        Vector2D finishPosition = new Vector2D(posX, posY);

                        Image image = new Image(imageName);

                        AxisAlignedBoundingBox volume = new AxisAlignedBoundingBoxImpl(finishPosition,
                                        height.orElse(image.getHeight()),
                                        height.map(h -> h * image.getWidth() / image.getHeight())
                                                        .orElse(image.getWidth()));

                        return new StaticEntityImpl(volume, Entity.Layer.FOREGROUND, image);
