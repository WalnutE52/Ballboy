    ...
    public Entity createEntity(Level level, JSONObject config) {
        try {
            ...
            KinematicState kinematicState = new KinematicStateImpl.KinematicStateBuilder().setPosition(startingPosition)
                    .setHorizontalVelocity(startVelocityX).build();

            Image image = new Image(imageName);

            AxisAlignedBoundingBox volume = new AxisAlignedBoundingBoxImpl(startingPosition,
                    height.orElse(image.getHeight()),
                    height.map(h -> h * image.getWidth() / image.getHeight()).orElse(image.getWidth()));

            CollisionStrategy collisionStrategy = new EnemyCollisionStrategy(level, color);
            ...
            case "aggressive":
                ...
                break;
            default:
                throw new ConfigurationParseException(String.format("%s is not a valid entity behaviour\n", behaviour));
            }

            return new DynamicEntityImpl(kinematicState, volume, Entity.Layer.FOREGROUND, image, collisionStrategy,
                    behaviourStrategy);
