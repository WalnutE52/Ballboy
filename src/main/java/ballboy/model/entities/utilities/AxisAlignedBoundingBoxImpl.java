...
public class AxisAlignedBoundingBoxImpl implements AxisAlignedBoundingBox {
    ...
    public AxisAlignedBoundingBoxImpl(Vector2D topLeft, double height, double width) {
        this.width = width;
        this.height = height;
    ...
    public boolean collidesWith(AxisAlignedBoundingBox box) {
        return getLeftX() < box.getRightX() && getRightX() > box.getLeftX() && getBottomY() > box.getTopY()
                && getTopY() < box.getBottomY();
    }

    @Override
    public boolean containsPoint(Vector2D point) {
        return point.isRightOf(getLeftX()) && point.isLeftOf(getRightX()) && point.isAbove(getBottomY())
                && point.isBelow(getTopY());
    }

    @Override
    public AxisAlignedBoundingBoxImpl copy() {
        return new AxisAlignedBoundingBoxImpl(topLeft, height, width);
    }
}