...
public class PhysicsEngineImpl implements PhysicsEngine {
    ...
    @Override
    public void resolveCollision(DynamicEntity a, StaticEntity b, Level level) {
        ...
        /*
         * collided prior to the most recent update, so no need to resolve the collision
         * as it would have been delt with before
         */
    ...
    @Override
    public void resolveCollision(DynamicEntity a, DynamicEntity b) {
        if ((a.getLayer() == Entity.Layer.BACKGROUND || b.getLayer() == Entity.Layer.BACKGROUND)) {
            return;
        }

        /*
         * if they collided in a previous update cycle then dont handle it, as it would
        ...
        /*
         * step the previous volume only horizontally. If there is a collision it is
         * treated as if the objects collided in the horizontal axis. If not, then it is
         * treated as if the objects collided vertically
         */
        ...
        if (isHorizontalCollision) { // horizontal collision
            ...
            double aVelocityXNew = ((aMass - bMass) / (aMass + bMass)) * aVelocityX
                    + ((2 * bMass) / (aMass + bMass)) * bVelocityX;
            double bVelocityXNew = ((2 * aMass) / (aMass + bMass)) * aVelocityX
                    - ((aMass - bMass) / (aMass + bMass)) * bVelocityX;
            ...
            double aVelocityYNew = ((aMass - bMass) / (aMass + bMass)) * aVelocityY
                    + ((2 * bMass) / (aMass + bMass)) * bVelocityY;
            double bVelocityYNew = ((2 * aMass) / (aMass + bMass)) * aVelocityY
                    - ((aMass - bMass) / (aMass + bMass)) * bVelocityY;
    ...
    @Override
    public void enforceWorldLimits(DynamicEntity a, Level level) {
        // if outside horizontal boundaries and travelling in the wrong direction
        if (a.getPosition().isRightOf(level.getLevelWidth()) && a.getVelocity().getX() > 0
                || a.getPosition().isLeftOf(0.0) && a.getVelocity().getX() < 0) {
            reflectX(a);
        }
    ...
    private void reflectX(DynamicEntity entity) {
        entity.setVelocity(entity.getVelocity()
                .addX(-1 * entity.getHorizontalAcceleration() * frameDurationMilli * 1e-3).reflectX());
    }
     ...
     */
    private void reflectY(DynamicEntity entity, double levelGravity) {
        entity.setVelocity(entity.getVelocity().addY(-1 * levelGravity * frameDurationMilli * 1e-3).reflectY());
    }
}
