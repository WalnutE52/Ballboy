...
import ballboy.model.Caretaker;
...
class KeyboardInputHandler {
    ...
    KeyboardInputHandler(GameEngine model) {
        ...
        // TODO (bobbob): Move sound choice/production into the model before alpha is
        // released to the new devs
        // TODO (bobbob): Just commenting this out while I debug my driver - don't
        // forget to revert this before anyone
        // else sees this
    ...
    void handlePressed(KeyEvent keyEvent) {
        ...
        if (keyEvent.getCode().equals(KeyCode.R)) {
            this.model.cheatMode();
        }
        if (keyEvent.getCode().equals(KeyCode.Q)) {
            Memento m = caretaker.uploadMemento();
            this.model.loadGame(m);
        }
        if (keyEvent.getCode().equals(KeyCode.S)) {
            Memento m = this.model.storeGame();
            caretaker.storeMemento(m);
        }
